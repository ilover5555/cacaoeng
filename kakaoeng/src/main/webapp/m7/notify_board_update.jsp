...
function register(){
	   ...
	   $.ajax({
		   type:'POST',
		   url:makeUrl('./${boardName}BoardUpdate.do'),
		   contentType:'application/x-www-form-urlencoded; charset=UTF-8',
		   datatype: 'text/plain',
		   data : s,
		   success: function(data){
			   alert(data);
			   window.location = "./${boardName}BoardView.view?boardId=${param.boardId}&page=${param.page}&viewPerPage=${param.viewPerPage}";
			   },
...
<%-- 
	<c:if test="${(null eq sessionScope.student) and (null eq sessionScope.admin) and (null eq sessionScope.exe) }">
	...
	<jsp:include page="../include/header.jsp" flush="false"></jsp:include>
		...
		<div class="btn-toolbar" data-role="editor-toolbar"
			...
			<div style="width: 100%; margin-bottom: 5px;" class="input-group">
				<input id="title" style="border-radius : 5px; height: 30px;" type="text" class="form-control" placeholder="제목" value="${Board.title }">
			</div>
			<div id="editor"  style="height:300px; max-height: 400px;">${Board.contents }</div> 
		...
		<div style="overflow: hidden;">
			...
			<button style="width:136px; height:39px; background-image: url('../artifact/student_boardwrite_register.png'); border: none; float:right;" onclick="register(); return false;"></button>
		</div>
