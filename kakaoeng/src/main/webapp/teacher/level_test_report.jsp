...
<script>
	$(document).ready(function(){
		$('#submitButton').click(function(){
			...
			s["status"] = status;
			if(status == 'LevelTestUncompleted'){
				s["reason"] = $('textarea[name="reason"]').val();
			}else if(status == 'LevelTestCompleted'){
				s["reason"] = $('textarea[name="assessment"]').val();
			}else{
				alert('Invalid');
				return;
			}
...
<body>
	<div class="content" style="width: 500px; margin: 0 auto">
			<div style="width: 100%; float: left" class="form-group">
				...
				<p style="text-align: center; text-decoration: underline; font-size: x-large; margin-bottom: 10px;">
					Level Test (Demo Class) Report
				</p>
				...
				<div style="width: 100%; border: 1px solid #c0c0c0; padding: 5px;"
					class="input-group">
					<div class="radio">
						<label><input type="radio" name="status" value="LevelTestCompleted" <c:if test="${classLog.classState eq 'LevelTestCompleted' }">checked="checked"</c:if> />Completed</label>
					</div>
					<div style="width: 100%" class="input-group">
						<span style="width: 110px" class="input-group-addon">
							Assessment
						</span>
						<textarea rows="5" class="form-control" name="assessment"><c:if test="${classLog.classState eq 'LevelTestCompleted' }">${classLog.reason }</c:if></textarea>
					</div>
				</div>
				...
				<div style="width: 100%; border: 1px solid #c0c0c0; padding: 5px;"
					...
					<div class="radio">
						<label><input type="radio" name="status"
							value="LevelTestUncompleted" <c:if test="${classLog.classState eq 'LevelTestUncompleted' }">checked="checked"</c:if> />Uncompleted</label>
					</div>
					<div style="width: 100%" class="input-group">
						<span style="width: 110px" class="input-group-addon">
							Reason for <br /> Uncompleted
						</span>
						<textarea rows="5" class="form-control" name="reason"><c:if test="${classLog.classState eq 'LevelTestUncompleted' }">${classLog.reason }</c:if></textarea>
					</div>
				</div>
