...
<script>
	$(document).ready(function(){
		$('#prev').click(function(){
			...
			document.location = './viewTeacherScheduleState.do?baseDate='+result;
		});
		$('#next').click(function(){
			var d = new Date($('#dateHolder').val());
			d.setTime(d.getTime() + (7*24*60*60*1000));
...
<div style="overflow: hidden; margin-left: auto; margin-right: auto;">
	<i id="prev" data-id="${teacher }" class="fa fa-arrow-circle-left" style="font-size: 50px; float:left; cursor: pointer; margin-left: 1px; color: rgb(56,143,212)"></i>
	<i id="next" class="fa fa-arrow-circle-right" style="font-size: 50px; float:right; cursor: pointer; color: rgb(56,143,212)"></i>
	<p style="text-align: center; font-size: 30px; line-height: 50px;">Weekly Schedule</p>
</div>
				...
				<c:forEach var="dvo" items="${DVOList }">
					<th class="table_header_column" style="background-color: ${dvo.backgroundColor}" data-date="${dvo.dateForm }">
						<p style="text-align: center; color:${dvo.color}"  <c:if test="${dvo.holiday eq true }">class="holidayP"</c:if>>
							<c:if test="${dvo.count > 0 }"><a href="./viewOneDaySchedule.view?baseDate=${dvo.dateForm }" style="color: inherit;"></c:if>
								${dvo.dateForm } <br/> ${dvo.dayOfWeek }
							<c:if test="${dvo.count > 0 }"></a></c:if>
						</p>
					</th>
				</c:forEach>
				</tr>
